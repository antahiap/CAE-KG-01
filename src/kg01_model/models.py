...
class GrpFts(neomodel.StructuredRel):
    ...
    '''
    contain the properties for the grouped parts 
    related to the simulation, the _e_ in naming is 
    refering to the edgeproperty

        type    describe the method
                box_mrg
        key     lis of name of the variable
                nrg_max
...
class Sim(neomodel.StructuredNode):
    ...
    # sim_reference = neomodel.Relationship('Sim', 'SIM_BASED_ON')
    # sim_part = neomodel.RelationshipTo('Part', 'INCL_PART')
    sim_des = neomodel.RelationshipTo('Des', 'SIM_DES', model=WgtRel)
    # sim_behav = neomodel.RelationshipTo('Behav', 'SIM_BEHAV')
    # sim_atr = neomodel.RelationshipTo('Atr', 'SIM_ATTR')
...
class Part(neomodel.StructuredNode):
    ...
    # Behavrt_part = neomodel.RelationshipTo('Part', 'CNCT_TO') # shramana CONNECTED_TO
...
class Mthd(neomodel.StructuredNode):
    ...
    can be connected to part, node and element. that reduces their
    properties in time to some features. 
        ...
        fts_keys      list of name of features that are linked to this node 
    '''
    uid = neomodel.UniqueIdProperty()
